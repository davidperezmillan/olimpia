...
class TelegramChatIds(object):
    ...
    def __str__(self):
        ...
        if self.group:
            x.append('surname={0}'.format(self.group))            
        return ' '.join(x)
...
class TelegramNotifier(object):
    ...
    def _real_init(self, client):
        ...
        logger.info('token=%s, parse_mode=%s, usernames=%s, fullnames=%s, groups=%s', self._token,
    ...
    def _check_token(self):
        try:
            ...
            logger.info('bot.getMe() raised: %s', repr(e))
            raise Exception('invalid bot token')
        except (NetworkError, TelegramError) as e:
            logger.error('Could not connect Telegram servers at this time, please try again later: %s', e.message)
    ...
    def _send_msgs(self, msg, chat_ids):
        ...
        for chat_id in (x.id for x in chat_ids):
            try:
                logger.info('sending msg to telegram servers: %s', msg)
                self._bot.sendMessage(chat_id=chat_id, text=msg, **kwargs)
            except TelegramError as e:
                if kwargs.get('parse_mode'):
                    logger.warning('Failed to render message using parse mode %s. Falling back to basic parsing: %s',
    ...
    def _get_chat_ids_n_update_db(self):
        ...
        if not chat_ids:
            ...
            if usernames:
                logger.warning('no chat id found for usernames: %s', usernames)
            if fullnames:
                logger.warning('no chat id found for fullnames: %s', fullnames)
            if groups:
                logger.warning('no chat id found for groups: %s', groups)
    ...
    def _get_chat_ids(self, usernames, fullnames, groups):
        ...
        logger.debug('found {0} cached chat_ids: {1}'.format(len(chat_ids), ['{0}'.format(x) for x in chat_ids]))
        ...
        logger.debug('found {0} new chat_ids: {1}'.format(len(new_chat_ids), ['{0}'.format(x) for x in new_chat_ids]))
    ...
    def _get_new_chat_ids(self, usernames, fullnames, groups):
        ...
        logger.debug('upd_usernames {} : {}, upd_fullnames {} : {}, upd_groups {} : {}'. format(upd_usernames,usernames, upd_fullnames, fullnames, upd_groups, groups))
        ...
        for i, username in enumerate(reversed(usernames)):
            ...
            if chat is not None:
                logger.debug('recuperando {}: id {}, username {} first_name {}, surname {} '. format(chat,chat.id,chat.username,chat.first_name, chat.last_name))
        ...
        for i, grp in enumerate(reversed(groups)):
            ...
            if chat is not None:
                logger.debug('recuperando {}: id {}, group {} '. format(chat,chat.id,chat.title))
    ...
    def _get_bot_updates(self):
        ...
        for update in updates:
            ...
            if chat.type == 'private':
                ...
                logger.warning('unknown chat type: %s}', type(chat))
